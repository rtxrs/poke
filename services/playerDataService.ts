...
import { DATA_PATH, DATA_FOLDER, RANKINGS_FILE, DATA_DIR, PUBLIC_ID_MAP_FILE } from '../config.js';
...
export interface PokemonRarity {
    score: number;
    breakdown: {
        iv: { value: number, text: string };
        shiny: { value: number, text: string };
        lucky: { value: number, text: string };
        origin: { value: number, text: string };
    };
}
...
export interface Rankings {
    recentPlayers: PlayerRankingSummary[];
    strongestPokemon: PokemonRankData[];
    rarestPokemon: PokemonRankData[];
    lastHeavyUpdate: number;
}

const playerDataService = {
    publicIdMap: new Map<string, string>(), // Stores publicId -> playerId mapping
    playerIdToPublicIdMap: new Map<string, string>(), // Stores playerId -> publicId mapping
    _initialized: false,
    rankingsCache: null as Rankings | null,
    _rankingsJob: null as any,

    async init() {
        if (this._initialized) return;
        
        if (!uuidv4) {
            const uuidModule: any = await import('uuid');
    ...
    async generatePublicId(playerId: string): Promise<string> {
        await this.init(); 
    ...
    async getPlayerIdFromPublicId(publicId: string): Promise<string | undefined> {
        await this.init();
    ...
    async _calculatePokemonRankings(playerFiles: string[], playerIdToUserId: Map<string, string>) {
        let totalPokemonEvaluated = 0;
        const getPokedexEntry = (p: any) => {
        ...
        const strongestCandidates: PokemonRankData[] = [];
        const rarestCandidates: PokemonRankData[] = [];

        const addToCandidateList = (list: any[], item: any, scoreField: string, max = 50) => {
            list.push(item);
        ...
        for (const file of playerFiles) {
            try {
                ...
                content.pokemons.forEach((p: any) => {
                    ...
                    if (p.pokemonDisplay?.shiny) {
                        if (p.pokemonDisplay.isStrongPokemon) {
                            ...
                            rarity.breakdown.shiny.text = (p.originEvents && p.originEvents.some((event: string) => event.includes('community_day'))) ? 'Community Day' : 'Shiny';
                        }
                    }
                    ...
                    const baseData: PokemonRankData = {
    ...
    async generateAndSaveRankings(): Promise<Rankings> {
        await this.init();
        ...
        try {
            ...
            let recentPlayers: PlayerRankingSummary[] = [];
            try {
                const existing = JSON.parse(await fs.readFile(RANKINGS_FILE, 'utf-8'));
                ...
                for (let i = 0; i < playerFiles.length; i++) {
                    ...
                    const buddy = content.pokemons.find((p: any) => p.id === content.account.buddyPokemonProto?.buddyPokemonId);
                    recentPlayers.push({
    ...
    async updateRecentPlayersOnly(playerData: any) {
        ...
        let rankings: Rankings = { recentPlayers: [], strongestPokemon: [], rarestPokemon: [], lastHeavyUpdate: 0 };
        try {
            rankings = JSON.parse(await fs.readFile(RANKINGS_FILE, 'utf-8'));
        } catch (e) { return; }

        const buddy = playerData.pokemons.find((p: any) => p.id === playerData.account.buddyPokemonProto?.buddyPokemonId);
        const newEntry: PlayerRankingSummary = {
            name: playerName,
            playerId: playerId,
            publicId,
            userId,
    ...
    async getPublicPlayerSummaries(): Promise<any[]> {
        await this.init();
        try {
            const files = await fs.readdir(DATA_PATH);
            return await Promise.all(
                files.filter(f => f.endsWith('.json') && f !== 'PGSStats.json').map(async file => {
                    ...
                    let displayPokemon = content.pokemons.find((p: any) => p.id === content.account?.buddyPokemonProto?.buddyPokemonId);
                    if (!displayPokemon) displayPokemon = content.pokemons.filter((p: any) => !p.isEgg).sort((a: any, b: any) => b.cp - a.cp)[0];
    ...
    async getPlayerDetail(playerId: string): Promise<any> {
        ...
        try {
            ...
            const allPokemon = content.pokemons.filter((p: any) => !p.isEgg);
            
            let latestCaught = null;
            let latestShiny = null;
            const topStrongest: any[] = [];
    ...
    async getPrivatePlayerData(playerId: string): Promise<any> {
        await this.init();
        try {
