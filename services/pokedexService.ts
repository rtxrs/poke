...
import {
    POKEDEX_API_URL, POKEDEX_FILE, POKEDEX_SERVER_FILE, POKEDEX_CLIENT_FILE,
    DATA_DIR, SHINY_RATES_FILE, POKEDEX_RAW_FILE, COSTUME_ID_MAP_FILE,
    MOVE_ID_MAP_FILE, FAST_MOVES_FILE, CHARGED_MOVES_FILE,
    TYPE_EFFECTIVENESS_FILE, TYPE_EFFECTIVENESS_API_URL, STATUS_FILE
} from '../config.js';
...
const CLIENT_MAPPING: { [key: string]: string } = {
...
export interface PokedexEntry {
    ...
    formId?: string;
    primaryType?: { type: string, names: { English: string } };
    secondaryType?: { type: string, names: { English: string } };
    pokemonClass?: string;
    assetForms?: any[];
    regionForms?: { [key: string]: PokedexEntry };
    [key: string]: any;
}
...
const pokedexService = {
    pokedex: null as { [key: number]: { [key: string]: PokedexEntry } } | null,
    moveMap: {} as { [key: string]: { name: string, type: string } },
    shinyRates: null as { [key: string]: number } | null,
    shinyPokemonTiers: null as { [key: string]: string } | null,
    defaultShinyTier: 'standard',
    costumeIdMap: {} as { [key: string]: string },
    typeColorMap: {
    ...
    async checkForPokedexUpdates(): Promise<boolean> {
        let originalPokedexContent: Buffer | null = null;
        try {
            ...
            const hashes: any = await hashesResponse.json();
    ...
    async checkForMoveUpdates(): Promise<boolean> {
        let updated = false;
        try {
            ...
            const remoteHashes: any = await hashesResponse.json();
    ...
    async checkForTypeEffectivenessUpdate(): Promise<boolean> {
        let updated = false;
        try {
    ...
    async getHealthCheckData(): Promise<HealthStatus> {
        try {
            const raidBossStatusContent = await fs.readFile(STATUS_FILE, 'utf-8');
            const raidBossStatus: any = JSON.parse(raidBossStatusContent);
    ...
    async _processAndLoadPokedex(): Promise<void> {
        try {
            const rawPokedexJson = await fs.readFile(POKEDEX_RAW_FILE, 'utf-8');
            const rawData: any[] = JSON.parse(rawPokedexJson);
            ...
            const cleanedData = rawData.map(pokemon => {
                ...
                if (pokemon.assetForms && Array.isArray(pokemon.assetForms)) {
                    pokemon.assetForms.forEach((assetForm: any) => {
                        if (assetForm.form) {
        ...
        try {
            ...
            const data: PokedexEntry[] = JSON.parse(serverPokedexJson);
            this.pokedex = {};
        ...
        try {
            ...
            const fastMoves: any[] = JSON.parse(fastMovesContent);
            const chargedMoves: any[] = JSON.parse(chargedMovesContent);
    
            const finalMoveMap: { [key: string]: { name: string, type: string } } = {};
        ...
        try {
            ...
            const shinyRatesData: any = JSON.parse(shinyRatesContent);
    ...
    regeneratePvPRanks(): Promise<void> {
    ...
    getShinyRate(pokemonId: number, origin: number, pokemonClass: string | undefined, originEvents: string[] | undefined): number {
    ...
    getPokemonName(dexNr: number, formName: string): string {
    ...
    getPokemonSprite(p: any): string {
        ...
        let costumeKey: string | null = null;
        ...
        const findAsset = (costume: string | null, form: string | null) => {
            let match = basePokemon.assetForms!.find((asset: any) => 
            ...
            match = basePokemon.assetForms!.find((asset: any) => 
               (asset.costume === costume || (!asset.costume && !costume)) &&
               (asset.form === form || (!asset.form && !form)) &&
            ...
            if (form || costume) {
                return basePokemon.assetForms!.find((asset: any) => 
                   (asset.costume === costume || (!asset.costume && !costume)) &&
                   (asset.form === form || (!asset.form && !form))
                );
            }
        ...
        if (!foundAsset) {
            foundAsset = basePokemon.assetForms!.find((asset: any) => asset.form === 'NORMAL' && !asset.costume && (asset.isFemale === undefined || asset.isFemale === isFemale));
        }

        return foundAsset?.[targetSprite] || (p.pokemonDisplay.shiny ? shinySprite : defaultSprite);
    },

    getPokemonTypeColors(pokedexEntry: PokedexEntry | null | undefined): string[] {
        const colors = [];
