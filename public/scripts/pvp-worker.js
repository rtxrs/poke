...
function generateRankTable(baseStats) {
    const ranks = {
        great: [],
        ultra: [],
    ...
    // Sort levels numerically to ensure consistent "lowest level for max stats" or similar tie-breaking if needed
    // Filter out levels above 50 (standard max level, excluding Best Buddy for general ranking matches PGSharp)
    ...
    for (let atk = 0; atk <= 15; atk++) {
        for (let def = 0; def <= 15; def++) {
            for (let sta = 0; sta <= 15; sta++) {
                ...
                const glStats = findOptimalStats(baseStats, ivs, LEAGUES.GREAT, sortedLevels);
                ranks.great.push({ ivs: `${atk}/${def}/${sta}`, ...glStats });

                // Calculate for Ultra League (Cap 2500)
                const ulStats = findOptimalStats(baseStats, ivs, LEAGUES.ULTRA, sortedLevels);
                ranks.ultra.push({ ivs: `${atk}/${def}/${sta}`, ...ulStats });

                // Calculate for Master League (No Cap - effectively Level 50)
                // Using findOptimalStats ensures we find the best level (max 50) just like other leagues
                const mlStats = findOptimalStats(baseStats, ivs, LEAGUES.MASTER, sortedLevels);
...
function findOptimalStats(baseStats, ivs, cap, sortedLevels) {
...
self.onmessage = function(e) {
    ...
    pokemons.forEach(p => {
        ...
        if (!speciesCache[speciesKey]) {
            speciesCache[speciesKey] = generateRankTable(entry.stats);
        }
        ...
        // This handles ties by giving them the same rank (e.g. Joint 1st)
        const getRank = (list, myStatProduct) => {
            // Find the first item that is NOT strictly greater (i.e. <=)
            // Since list is sorted Descending, this gives us the count of strictly greater items.
            const countGreater = list.findIndex(r => r.statProduct <= myStatProduct);
            if (countGreater === -1) return list.length + 1; // Should not happen if myStatProduct is in list
        ...
        const glRank = glEntry ? getRank(rankTables.great, glEntry.statProduct) : null;
        ...
        const ulRank = ulEntry ? getRank(rankTables.ultra, ulEntry.statProduct) : null;
