...
export default async function scrapeMaxBattles() {
    ...
    const getPokemonDetails = (id) => {
        ...
        if (!image && entry.assetForms) {
            const defaultAsset = entry.assetForms.find((a) => (a.form === 'NORMAL' || !a.form) && (a.costume === 'NONE' || !a.costume));
            image = defaultAsset ? defaultAsset.image : entry.assetForms[0].image;
        }
        const types = [];
        if (entry.primaryType?.names?.English)
            types.push(entry.primaryType.names.English);
        if (entry.secondaryType?.names?.English)
            types.push(entry.secondaryType.names.English);
        return {
